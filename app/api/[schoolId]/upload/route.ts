...
import { GenericApiParams } from "@/types/api";
...
export async function POST(
  ...
  try {
    ...
    if (!session?.user) return new NextResponse("Unauthenticated", { status: 403 });

    const formData = await req.formData();
    const file = formData.get("file") as File;
    const providedSchoolId = formData.get("schoolId")?.toString() || schoolId;
    ...
    if (!file.type.includes("excel") && !file.name.endsWith(".xlsx")) {
      return new NextResponse("Invalid file type. Please upload an Excel file.", { status: 400 });
    }
    ...
    if (!worksheet) {
      return new NextResponse("Invalid Excel file. No worksheet found.", { status: 400 });
    }
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      if (!name || !email || !pin || !phone || !genreValue) {
        throw new NextResponse(`Missing data in row ${rowNumber}`, { status: 400 });
      }

      if (!Object.values(Genre).includes(genreValue as Genre)) {
        throw new NextResponse(`Invalid genre in row ${rowNumber}. Must be "Male" or "Female".`, { status: 400 });
      }
